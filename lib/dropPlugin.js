...
var config      = '/config/drop/drop';  // 配置文件位置
...
var DropPlugin = function() {
    this.name = 'dropPlugin';
    var dropJsonAddress = require(path.join(pomelo.app.getBase(), config))[pomelo.app.get('env')];
    var dropJson = require(path.join(pomelo.app.getBase(), dropJsonAddress));
    this.dropObjMap = {};
    for (let key in dropJson) {
        ...
        this.dropObjMap[key] = new DropObj(dropJ);
    }
...
module.exports = new DropPlugin();
var pro = DropPlugin.prototype;
/**
 * 根据掉落组ID,获得掉落
 * @param dropId
 * @returns {Array}
